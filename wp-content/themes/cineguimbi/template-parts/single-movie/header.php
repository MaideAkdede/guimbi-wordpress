<div class="h-1/2 text-white mt-10vh md:h-75vh lg:h-78vh lg:grid lg:place-content-center">
    <div class="relative px-5 mx-auto lg:static lg:grid lg:grid-cols-movie-lg xl:gap-24 xxl:grid-cols-movie lg:items-center max-w-screen xl:max-w-movie lg:gap-4">
        <div class="xxl:col-start-1 lg:justify-self-start">
            ...
            <a href="#info" title=""
               class="hidden px-6 text-sm font-bold tracking-widest uppercase duration-150 ease-in lg:inline-block bg-primary font-family-bold py-15px mb-15 hover:text-primary shadow-set-hover hover:shadow-hover-white">En savoir plus<span class="sr-only"> sur <?= the_title(); ?></span> <i class="ml-2.5 fa-regular fa-angle-down"></i>
            </a>
        </div>
        <a class="flex absolute top-1/2 left-1/2 flex-col justify-center items-center bg-white rounded-full duration-150 -translate-x-1/2 lg:static lg:translate-x-0 xxl:col-start-3 lg:justify-self-center w-play lg:w-play-lg aspect-square lg:bg-white/10 hover:bg-primary fill-primary lg:fill-white/50 hover:fill-white/100 lg:justify-start"
        ...
        <?php if (has_post_thumbnail()): ?>
            <div class="mx-auto w-auto xxl:col-start-4 xxl:justify-self-end lg:z-1 xl:w-80vw max-w-movie-image">
                <?= get_the_post_thumbnail(null, 'medium_large', ['class' => 'top-0 xxl:mt-36 w-full h-full object-contain object-top']); ?>
            </div>
        <?php endif; ?>
    </div>
</div>